"""
Statistical methods for scRNA-seq analysis.

Provides mathematically rigorous methods for:
- Adaptive QC threshold selection using robust statistics
- Variance-aware normalization
- Explained variance-based PC selection
- Full differential expression analysis

All methods are designed for reproducibility and interpretability.
"""
...
class PCSelectionResult:
    ...
    n_pcs_selected: int
    method: str  # "elbow", "variance_ratio", "jackstraw"
...
class RunMetadata:
    ...
    cells_before_qc: int = 0
    cells_after_qc: int = 0
    genes_before_qc: int = 0
    genes_after_qc: int = 0

    # Normalization
    normalization_method: str = "size_factor"
    ...
    pc_selection: Optional[Dict[str, Any]] = None
    n_pcs_used: int = 0
    n_neighbors: int = 15
    ...
    deg_correction: str = "benjamini-hochberg"
...
def apply_adaptive_qc(
    ...
    stats = {
        ...
        'removed_low_genes': int(n_removed_min_genes),
        'removed_high_genes': int(n_removed_max_genes),
        'removed_low_counts': int(n_removed_min_counts),
        'removed_high_counts': int(n_removed_max_counts),
        'removed_high_mt': int(n_removed_mt),
    }
...
def select_pcs_elbow(
    ...
    1. Compute second derivative of cumulative variance
    2. Find point of maximum curvature (elbow)
...
def run_full_deg_analysis(
    ...
    """
    Run full differential expression analysis with proper statistical output.

    Uses one-vs-rest comparison for each group and applies multiple testing
    correction using Benjamini-Hochberg procedure.
    ...
    Args:
        ...
        min_fold_change: Minimum log2 fold change for reporting
        min_pct: Minimum fraction of cells expressing gene

    Returns:
        Full DEG table with all statistics
    """
    logger.info(f"Running DEG analysis: method={method}, correction={corr_method}")

    # Run scanpy's rank_genes_groups with all genes
    ...
    for group in groups:
        ...
        names = adata.uns['rank_genes_groups']['names'][group_str]
        scores = adata.uns['rank_genes_groups']['scores'][group_str]
        pvals = adata.uns['rank_genes_groups']['pvals'][group_str]
        pvals_adj = adata.uns['rank_genes_groups']['pvals_adj'][group_str]
        logfc = adata.uns['rank_genes_groups']['logfoldchanges'][group_str]

        # Get percentage expressed
        pts = adata.uns['rank_genes_groups']['pts'][group_str]
        pts_rest = adata.uns['rank_genes_groups']['pts_rest'][group_str]
        ...
        for i, gene in enumerate(names):
            results.append({
                ...
                'score': scores[i],
                'pct_in': pts[i] * 100,  # Percentage in cluster
                'pct_out': pts_rest[i] * 100,  # Percentage outside
                'rank': i + 1
            })

    deg_df = pd.DataFrame(results)

    # Add statistical significance flags
    deg_df['significant'] = (
        (deg_df['pval_adj'] < 0.05) &
        (np.abs(deg_df['log2fc']) >= min_fold_change)
    )

    # Sort by cluster and adjusted p-value
    deg_df = deg_df.sort_values(['cluster', 'pval_adj'])

    n_sig = deg_df['significant'].sum()
    logger.info(f"DEG analysis complete: {n_sig} significant genes")

    return deg_df


def compute_qc_distributions(
    adata: sc.AnnData
...
def generate_qc_rationale(
    ...
    lines = [
        ...
        "",
        "**Mathematical Basis**: MAD (Median Absolute Deviation) is used instead of "
        "standard deviation because it is robust to outliers. For normally distributed data, "
        "3 MADs corresponds to approximately ±4.5σ, capturing 99.7% of the distribution.",
    ]

    return "\n".join(lines)


def generate_pc_selection_rationale(result: PCSelectionResult) -> str:
    """
    Generate human-readable rationale for PC selection.
    """
    cum_var = result.cumulative_variance[result.n_pcs_selected - 1] * 100

    lines = [
        "### PC Selection Rationale",
        "",
        f"**Method**: {result.method}",
        f"**PCs selected**: {result.n_pcs_selected}",
        f"**Cumulative variance explained**: {cum_var:.1f}%",
        "",
    ]

    if result.method == "elbow":
        lines.extend([
            "The elbow method identifies the point of maximum curvature in the ",
            "cumulative variance explained plot. This represents the point where ",
            "adding more PCs provides diminishing returns.",
            "",
            f"Elbow point detected at PC {result.elbow_point}."
        ])
    elif result.method == "variance_ratio":
        lines.extend([
            f"PCs were selected to explain at least {result.variance_threshold*100:.0f}% ",
            "of the total variance. This ensures that the major sources of variation ",
            "in the data are captured.",
            "",
            f"Elbow point for reference: PC {result.elbow_point}."
        ])

    # Add variance breakdown
    lines.extend([
        "",
    ...
    for i in range(min(10, len(result.variance_explained))):
        var = result.variance_explained[i] * 100
        cum = result.cumulative_variance[i] * 100
        lines.append(f"| {i+1} | {var:.2f} | {cum:.1f} |")
